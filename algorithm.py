...
def dfs(maze):
    ...
    dfs_queue = []
    gone = set()
    dfs_queue.append([maze.getFirst()])
    while dfs_queue:
        currentPath = dfs_queue.pop(-1)
        currentRow, currentCol = currentPath[-1]
        if (currentRow, currentCol) in gone:
            continue
        gone.add((currentRow, currentCol))
        ...
        for item in maze.dfs_getnear(currentRow, currentCol):
            if item not in gone:
                dfs_queue.append(currentPath + [item])
                # print([item])
    return [], 0
    ...
    while not pq.empty():
        ...
        for item in maze.getnear(currentRow, currentCol):
            ...
            if item not in gone:
                gone.add(item)
                Allcosts +=1 
                pq.put((Allcosts, currentPath + [item]))
    return [], 0
